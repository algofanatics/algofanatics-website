...
export async function POST(req: NextRequest) {
  try {
    ...
    const RegisterSchema = z.object({
      ...
      password: z.string().regex(/(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}/), //Password must contain digit, small letter, big letter and number
    })
    ...
    return NextResponse.json({ message: 'successful, you can now sign in' });
